...
export function timeSlotDue(slot: string, timezone: string): boolean {
 ...
 */
export function hoursSinceLastPost(lastPostedAt: Date | null | undefined): number {
...
export async function getVideoMetadata(url: string): Promise<{ title: string; description: string; tags: string[] }> {
  try {
 ...
 */
export async function downloadVideoToTempFile(url: string, label = "auto"): Promise<string> {
...
export async function fetchChannelIdFromHandle(handle: string): Promise<string | null> {
...
export async function fetchYouTubeRssVideos(
  channelId: string,
