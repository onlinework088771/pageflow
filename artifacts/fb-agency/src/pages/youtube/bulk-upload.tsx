...
function YtScheduleManager({
  ...
  /** When set, locks the Schedule Manager to this channel ID (from the wizard Step 1 selection). */
  activeChannelId?: string | null;
}) {
  const [confirmOpen, setConfirmOpen] = useState(false);
  // Internal channel filter — only used when no activeChannelId is provided by the wizard
  const [filterChannelId, setFilterChannelId] = useState<string>("all");

  // activeChannelId (from wizard) takes precedence; fall back to internal pill filter
  const effectiveChannelId = activeChannelId ?? filterChannelId;

  // Collect unique channel IDs present in the video list for the dropdown
  const channelOptions = Array.from(
    new Map(videos.map((v) => [v.channelId, channelMap.get(v.channelId)])).entries()
  ).filter(([, ch]) => ch !== undefined) as [string, YoutubeChannel][];

  const filteredVideos = effectiveChannelId === "all"
    ? videos
    : videos.filter((v) => v.channelId === effectiveChannelId);

  const channelStats = {
    total: filteredVideos.length,
    pending: filteredVideos.filter((v) => v.status === "pending" || v.status === "processing").length,
    posted: filteredVideos.filter((v) => v.status === "posted").length,
    failed: filteredVideos.filter((v) => v.status === "failed").length,
  };
  ...
  const counts = {
    ...
    pending: filteredVideos.filter((v) => v.status === "pending" || v.status === "processing").length,
    completed: filteredVideos.filter((v) => v.status === "posted").length,
    failed: filteredVideos.filter((v) => v.status === "failed").length,
  };
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        ...
        {/* Channel filter pills — only shown when the wizard has NOT locked a specific channel */}
        {!activeChannelId && channelOptions.length > 1 && (
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <span className="text-xs text-muted-foreground shrink-0">Filter by channel:</span>
            <div className="flex flex-wrap gap-1.5">
              <button
                onClick={() => setFilterChannelId("all")}
                className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${filterChannelId === "all" ? "bg-primary text-primary-foreground border-primary" : "border-border hover:border-primary/40 text-muted-foreground"}`}
              >
                All channels
              </button>
              {channelOptions.map(([id, ch]) => (
                <button
                  key={id}
        ...
        {/* When the wizard has locked a channel, show a read-only indicator */}
        {activeChannelId && (() => {
          const activeCh = channelMap.get(activeChannelId);
          return activeCh ? (
        ...
        {/* Summary counts for the active filter */}
        {!loading && filteredVideos.length > 0 && (
          <div className="grid grid-cols-4 gap-2 mt-3">
            {[
              { label: "Total", value: channelStats.total, cls: "bg-muted/50" },
              { label: "Pending", value: channelStats.pending, cls: "bg-yellow-50 dark:bg-yellow-950/30 border-yellow-200 dark:border-yellow-800" },
              { label: "Posted", value: channelStats.posted, cls: "bg-emerald-50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-800" },
              { label: "Failed", value: channelStats.failed, cls: "bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-800" },
            ].map(({ label, value, cls }) => (
              <div key={label} className={`rounded-lg border px-2 py-1.5 text-center ${cls}`}>
      ...
      <CardContent>
        <Tabs defaultValue="all">
          ...
          {tabs.map((tab) => {
            const filtered = filteredVideos.filter((v) => (tab.statuses as string[]).includes(v.status));
            return (
...
function YoutubeBulkUploadInner() {
  ...
  /* ── Metadata mode (step 3) ── */
  const [metadataMode, setMetadataMode] = useState<"single" | "manual">("manual");
  ...
  async function handleScheduleAll() {
    ...
    // Validation depends on the active metadata mode
    if (metadataMode === "single") {
      if (!singleTitle.trim()) {
        toast({ title: "Enter a title in the Single Metadata section", variant: "destructive" });
        return;
      }
    } else {
    ...
    try {
      for (const item of videoItems) {
        ...
        // Title & description: use shared values in single mode, per-video values in manual mode
        const title = metadataMode === "single" ? singleTitle.trim() : item.title.trim();
        const desc  = metadataMode === "single"
          ? singleDescription.trim()
          : [item.description.trim(), item.hashtags.trim()].filter(Boolean).join("\n\n");
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-0.5">
                    Choose how to apply titles and descriptions, then set privacy and schedule for each video.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {([
                    { mode: "single" as const, label: "Single Metadata", desc: "One title & description for all" },
                    { mode: "manual" as const, label: "Manual Per Video", desc: "Edit each video separately" },
                  ]).map(({ mode, label, desc }) => (
                    <button
                      key={mode}
                      type="button"
                    ...
                    >
                      <span className="text-sm font-semibold">{metadataMode === mode ? "●" : "○"} {label}</span>
                ...
                {metadataMode === "single" && (
                  <div className="rounded-xl border border-primary/30 bg-primary/5 p-4 space-y-3">
                    ...
                    <p className="text-xs text-muted-foreground -mt-1">
                      This title and description will be applied to every scheduled video.
                    </p>
                    <div className="space-y-1">
                      <Label className="text-xs">Title <span className="text-destructive">*</span></Label>
                      <Input
                        value={singleTitle}
                        placeholder="YouTube SEO title for all videos…"
                    ...
                    {/* Compact per-video privacy + schedule editors */}
                    <p className="text-xs font-medium text-muted-foreground pt-1">
                      Privacy &amp; schedule per video:
                    </p>
                    <div className="space-y-2 max-h-[40vh] overflow-y-auto pr-1">
                      {videoItems.map((item, index) => (
                        <div key={item.id} className="rounded-lg border border-border/60 bg-card p-3 space-y-2">
                          <div className="flex items-center gap-2">
                            <div className="h-8 w-12 rounded bg-muted flex items-center justify-center shrink-0 overflow-hidden border border-border/30">
                              {item.thumbnail
                                ? <img src={item.thumbnail} alt="" className="h-full w-full object-cover" />
                                : <Video className="h-4 w-4 text-muted-foreground/40" />}
                            </div>
                            <p className="text-xs font-medium truncate flex-1 text-muted-foreground">{item.file.name}</p>
                            <p className="text-[10px] text-muted-foreground/60 shrink-0">#{index + 1}</p>
                            <button onClick={() => removeVideo(item.id)} className="text-muted-foreground hover:text-destructive shrink-0 p-0.5">
                              <XCircle className="h-3.5 w-3.5" />
                            </button>
                          </div>
                          <div className="grid grid-cols-2 gap-2">
                            <div className="space-y-1">
                              <Label className="text-[10px]">Privacy</Label>
                              <Select value={item.privacy} onValueChange={(v) => updateVideo(item.id, { privacy: v as Privacy })}>
                                <SelectTrigger className="h-7 text-[11px] px-2 gap-1">
                                  <PrivacyIcon value={item.privacy} /><SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                  <SelectItem value="public">Public</SelectItem>
                            ...
                            <div className="space-y-1">
                              ...
                              <Select value={item.videoType} onValueChange={(v) => updateVideo(item.id, { videoType: v as VideoType })}>
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <div className="space-y-1">
                              <Label className="text-[10px] flex items-center gap-1"><Calendar className="h-2.5 w-2.5" />Date</Label>
                              <Input type="date" value={item.scheduledDate} onChange={(e) => updateVideo(item.id, { scheduledDate: e.target.value })} className="h-7 text-[11px]" />
                            </div>
                            <div className="space-y-1">
                              <Label className="text-[10px] flex items-center gap-1"><Clock className="h-2.5 w-2.5" />Time</Label>
                              <Input type="time" value={item.scheduledTime} onChange={(e) => updateVideo(item.id, { scheduledTime: e.target.value })} className="h-7 text-[11px]" />
                            </div>
                          </div>
                        </div>
                      ))}
